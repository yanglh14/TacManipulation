...
def run_sim(if_viewer, sim_length, num_iter, noise_scale):
    ...
    # pro = gym.get_actor_rigid_shape_properties(env, 1)
    # pro[0].friction = 0.0
    # gym.set_actor_rigid_shape_properties(env,1,pro)

    pro = gym.get_actor_rigid_shape_properties(env, 0)
    for i in range(len(pro)):
        pro[i].friction = 0
    gym.set_actor_rigid_shape_properties(env,0,pro)
    ...
    while i < num_iter:
        ...
        if step%50 == 0:
            tactile_.append(np.array(tactile))
            tac_pose_.append(np.array(tac_pose))
            object_pos_.append(np.array(rigid_body_states[0,-1,:7]))

        if step ==100:
            root_state_tensor[-1,-3] = 2
